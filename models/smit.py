"""
SMIT 3D segmentation backbone and supporting transformer building blocks.
"""
...
class SMIT_3D_Seg(nn.Module):
    ...
    def forward_debug(self, x_in):
        ...
        enc44 = out_feats[-1].mean(dim=(-1, -2, -3))   # torch.Size([4, 384, 8, 8, 8])  
        enc33 = out_feats[-2].mean(dim=(-1, -2, -3))   # torch.Size([4, 192, 16, 16, 16])
        enc22 = out_feats[-3].mean(dim=(-1, -2, -3))   # torch.Size([4, 96, 32, 32, 32])   
        enc11 = out_feats[-4].mean(dim=(-1, -2, -3))   # torch.Size([4, 48, 64, 64, 64])    

        return enc11, enc22, enc33, enc44, x
