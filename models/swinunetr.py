...
"""
SwinUNETR segmentation backbone and supporting windowed-transformer helpers.
"""
...
class SwinUNETR(nn.Module):
    ...
    def forward_debug(self, x_in):
        hidden_states_out = self.swinViT(x_in, self.normalize)
        return hidden_states_out[0].mean(dim=(-1, -2, -3)),\
            hidden_states_out[1].mean(dim=(-1, -2, -3)),\
                hidden_states_out[2].mean(dim=(-1, -2, -3)),\
                    hidden_states_out[3].mean(dim=(-1, -2, -3)),\
                        hidden_states_out[4].mean(dim=(-1, -2, -3)) 
